    ...
    while (fgets(buffer, sizeof(buffer), file) != NULL && i < data->num_amostras) {//le linha por linha
        ...
        while (token != NULL) {//iteracoes que vao indo palavra por palavra por causa da funcao token da biblioteca string.h
            if (j == 0) { 
                ...
                data->rotulos[i] = (target_val == 1.0) ? 1.0 : -1.0;//passa o rotulo (-1 ou +1) pra depois validar e ver se errou ou acertou
            }
            j++;
        ...
        for (int i = 0; i < data->num_amostras; i++) {//itera sobre cada amostra do dataset
            ...
            for (int direcao = -1; direcao <= 1; direcao += 2) {//teste dois possiveis casos, direcao = -1 ou direcao = 1
                ...
                for (int j = 0; j < data->num_amostras; j++) {//itera sobre todas as amostras j para calcular o erro - através dele detecta se é um bom stum
                    double predicao = (direcao == 1)//define a predicao do stump (-1 ou 1) para a amostra j
                        ? ((data->features[j][f] >= limiar_atual) ? 1.0 : -1.0)//se for o primeiro caso, se a feature f da amostra j for >= que o limiar atual: predicao=1, se nao =-1
                        : ((data->features[j][f] < limiar_atual) ? 1.0 : -1.0);//mesma logica para direção =1
    ...
    *num_stumps = 0;//quantidade de stumps inicial é igual a 0 ->nenhuma arvore de decisao foi criada ainda
    ...
    for (int t = 0; t < ITERACOES; t++) {//vai iterando pela quantidade maxima de iterações (stumps)
        ...
        for (int i = 0; i < train_data->num_amostras; i++) {
            double h_x = (current_stump.direcao == 1)
                ? ((train_data->features[i][current_stump.indice_caracteristica] >= current_stump.limiar) ? 1.0 : -1.0)//calcula a predicao (h_x) de acordo com o limiar do melhor stump
                : ((train_data->features[i][current_stump.indice_caracteristica] < current_stump.limiar) ? 1.0 : -1.0);
        ...
        for (int i = 0; i < train_data->num_amostras; i++) {//itera sobre todas as amostras
            double h_x = (current_stump.direcao == 1)
                ? ((train_data->features[i][current_stump.indice_caracteristica] >= current_stump.limiar) ? 1.0 : -1.0)//recalcula a mesma predicao feita antes
                : ((train_data->features[i][current_stump.indice_caracteristica] < current_stump.limiar) ? 1.0 : -1.0);
...
double erro_previsao(int TP, int TN, int FN, int FP) {
    ...
    return (total == 0) ? 0.0 : (double)(FP + FN) / total;//formual do erro
}
double precisao(int TP, int FP) {
    return (TP + FP == 0) ? 0.0 : (double)TP / (TP + FP);//formula da precisao
}
double recall(int TP, int FN) {
    return (TP + FN == 0) ? 0.0 : (double)TP / (TP + FN);//formula do recall
}
double f1_score(double precision, double recall) {
    return (precision + recall == 0) ? 0.0 : (2.0 * precision * recall) / (precision + recall);//formula do f1score
}
...
double classifica_amostra(double* sample_features, WeakLearner* ensemble, int num_stumps) {//aqui ele classifica a amostra (feature)
    ...
    for (int t = 0; t < num_stumps; t++) {//itera sobre todos os stumps criados
        double h_x = (ensemble[t].direcao == 1)
            ? ((sample_features[ensemble[t].indice_caracteristica] >= ensemble[t].limiar) ? 1.0 : -1.0)
            : ((sample_features[ensemble[t].indice_caracteristica] < ensemble[t].limiar) ? 1.0 : -1.0);
        final_score += ensemble[t].alpha * h_x;//vai somando a quantidade de votos de cada stump SOBRE A FEATURE EM DESTAQUE
    }
    return (final_score >= 0.0) ? 1.0 : -1.0;//se o stump for positivo -> nao tem parkinson, se é negativo, tem parkinson
}
